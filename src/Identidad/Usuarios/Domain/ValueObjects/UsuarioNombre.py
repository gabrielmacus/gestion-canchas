...
class UsuarioNombre(RequiredStringValueObject):
    ...
    def _ensure_is_between_2_and_50_characters(self) -> None:
        if len(self.value) < 2 or len(self.value) > 50:
            raise InvalidNombreException("El nombre del usuario debe tener entre 2 y 50 caracteres")
    
    def _ensure_is_alphabetic(self) -> None:
        if not self.value.replace(" ", "").replace("-", "").isalpha():
            raise InvalidNombreException("El nombre solo puede contener letras, espacios y guiones") 